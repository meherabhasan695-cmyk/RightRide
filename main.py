...
import folium       #Creating interactive map using ORS         
import os       #Logo
import webbrowser
...
# Try and except use to avoid crash during input 
...
def mode_time(dist_km, mode, traffic):  #Time 
...
def auto_fare(dist, ttime):
    if dist < 4.5:
        return 22.45076 + 10.95818 * dist
    return 30 + 11 * dist + 1.2 * ttime
...
def format_cost(c, mode):  #Range of fare
    if mode in ("CNG", "Bike"):
...
# Graph 
def show_charts(ranked, best, budget):
    """Display score + cost bar charts directly on screen."""
...
def open_map(source, destination, traffic):
    ...
    folium.Marker([lat1, lon1], popup=source,
                  icon=folium.Icon(color="green", icon="play")).add_to(fmap)   #Green Start
    folium.Marker([lat2, lon2], popup=destination,
                  icon=folium.Icon(color="red", icon="stop")).add_to(fmap)   #Red stop
...
def main():
    ...
    if source.lower() == destination.lower():
        print("Error: Source and destination cannot be the same.") #Common Sense
        return
...
#calculation
    ...
    show_charts(ranked, best, budget)
